...
#elif defined(_WIN64)
    // Windows
    if (fileExists("lib/addr2line64.exe")) {
        out << "lib/addr2line64.exe -f -i -C -s -p -e \"" << exceptions::getProgramNameForStackTrace() << "\"" << addrsStr;
    } else {
        out << "(addr2line64.exe unavailable; no stack trace produced)";
    }
#elif defined(_WIN32)
    // Windows
    if (fileExists("lib/addr2line.exe")) {
        out << "lib/addr2line.exe -f -i -C -s -p -e \"" << exceptions::getProgramNameForStackTrace() << "\"" << addrsStr;
    } else {
...
static void stanfordCppLibSignalHandler(int sig) {
    ...
    if (sig == SIGSEGV) {
        ...
        SIGNAL_DETAILS = "This can happen when you dereference a pointer to an invalid memory address\n*** (possibly out of bounds, deallocated, nullptr, or ...).";
    } else if (sig == SIGABRT) {
...
void GConsoleWindow::_initMenuBar() {
    ...
    const std::string ICON_STRIP_FILE = "lib/iconstrip.png";
...
/*static*/ const std::string GWindow::DEFAULT_ICON_FILENAME = "lib/splicon-large.png";
...
//     // do nothing, do not init this->row or this->col
// }
