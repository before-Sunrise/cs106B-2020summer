...
namespace {
    ...
    void checkIntegrityOf(const EncodedData& data) {
         ...
         * distinct characters.
         */
        if (data.treeBits.size() != data.treeLeaves.size() * 2 - 1) {
...
void writeData(EncodedData& data, ostream& out) {
    ...
    uint8_t modulus = (data.treeBits.size() + data.messageBits.size()) % 8;
    if (modulus == 0) modulus = 8;
    out.put(modulus);

    /* Bits themselves. */
    BitWriter writer(out);
    while (!data.treeBits.isEmpty()) writer.put(data.treeBits.dequeue());
...
EncodedData readData(istream& in) {
    ...
    for (int i = 0; i < 2 * charCount - 1; i++) {
        data.treeBits.enqueue(reader.get());
        bitsToRead--;
    }
...
ostream& operator<< (ostream& out, const EncodedData& data) {
    ...
    builder << "{treeBits:" << data.treeBits
