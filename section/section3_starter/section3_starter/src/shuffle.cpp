...
string randomShuffle(string input){
    ...
    // random chose
    int randomChosen = randomInteger(0, size - 1);
    char chosenChar = input[randomChosen];
    input = input.substr(0, randomChosen) + input.substr(randomChosen + 1);

    // recursive case
    return chosenChar + randomShuffle(input);
}
